...
// states.html loads bills-data.js (state bills); federal.html loads
// federal-bills-data.js (federalBillsData). This picks the dataset the
// current page actually loaded, so every displayed number below is computed
// from data — never hardcoded in HTML.
function activeDataset() {
    if (typeof federalBillsData !== 'undefined' && typeof billsData === 'undefined') {
        return federalBillsData;
    }
...
function populateStateFilter() {
    ...
    const states = [...new Map(
        activeDataset().map(bill => [bill.state, bill.stateName])
...
function applyFilters(options = {}) {
    ...
    currentFilteredBills = activeDataset()
...
function calculateStatistics() {
    const stats = {
        ...
        states: new Set(activeDataset().map(bill => bill.state)).size,
        enacted: 0,
        pending: 0,
        failed: 0,
        tier1: 0,
        tier2: 0,
        families: new Set(activeDataset().map(bill => bill.proposalFamily || bill.id)).size,
        byState: {},
        byCategory: {}
    };

    activeDataset().forEach(bill => {
...
function updateStateMap(filters) {
    ...
    const mapBills = activeDataset().filter(bill => billMatchesFilters(bill, filters, normalizedQuery, true));
